        ...
        try:
            ...
            while n <= 2:
                try:
                    ...
                    n += 1
                    main(
                        client, trt_tb, qr, presto_config, pg_config, sup_l, n, deciles_
                    )
                    break
                except Exception as e:
        ...
        with psycopg2.connect(**pg_config) as pg_conn:
            ...
            with pg_conn.cursor() as cursor:
                with open(d_file, "r") as f:
                    ...
                    cursor.copy_expert(
                        f"COPY {dtb} FROM STDIN WITH DELIMITER '|' CSV", f
                    )
                    pg_conn.commit()
                    if indx_val == indx_creation:
                        ...
                        if not result:
                            try:
                                ...
                                cursor.execute(
                                    f"create index {dtb}_email_idx on  {dtb} (email) "
                                )
                            except Exception as e:
                        ...
                        if sup_l == "True":
                            ...
                            if not mresult:
                                try:
                                    ...
                                    cursor.execute(
                                        f"create index {dtb}_md5_idx on  {dtb} (md5hash) "
                                    )
                                except Exception as e:
    ...
    try:
        ...
        with psycopg2.connect(**pg_config) as conn:
            ...
            with conn.cursor() as cursor:
                ...
                qry = [
                    item for item in qry if re.search(r"apt_rltp_request_raw_", item)
                ]
                indx_creation = len(qry)
                sqr = qry[0] + " LIMIT 3"
                if (
                    int(
                        re.findall("apt_rltp_request_raw_(\d+)_postback_file", qry[0])[
                            0
                        ]
                    )
                    > 28388
                ...
                try:
                    ...
                    logger.error(
                        "Unable to pull sample data from presto ::{}".format(e)
                    )
                    status_up("Unable to pull data from RLTP")
                    sys.exit(1)
                logger.info(samp)
                ...
                try:
                    if not inspector.has_table(trt_tb):
                        cursor.execute(
                            f"CREATE TABLE {trt_tb} ({colsn}) PARTITION BY LIST(decile)"
                        )
                        for i in list(df3["decile"].drop_duplicates()):
                            cursor.execute(
                                f"CREATE TABLE {trt_tb}_{i} PARTITION OF {trt_tb} FOR VALUES IN ('{i}')"
                            )
        ...
        for qr in qry:
            ...
            try:
                ...
                with Pool(
                    processes=5, initializer=init_worker, initargs=(shared_event,)
                ) as pool:
                    pool.map(
                        main,
                        [
