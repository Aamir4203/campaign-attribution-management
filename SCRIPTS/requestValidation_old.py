...
try:
    ...
    cur = conn.cursor()
    cur.execute(
        "update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_validation='V',request_desc='Validation Failed' where request_id="
        + sys.argv[1]
        + ""
    )
    df = pd.read_sql(
        "select * from APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND where request_id="
        + sys.argv[1]
        + "",
        con=engine,
    )
    df6 = pd.read_sql(
        "select * from APT_CUSTOM_CLIENT_INFO_TABLE_DND where client_id="
        + str(df["client_id"][0])
        + "",
        con=engine,
    )
    ...
    if os.path.isfile(df["cpm_report_path"][0]):
        ...
        if len(df2.columns) == 14:
            ...
            if (
                not df2["Delivered"].apply(validate_dtype).all()
                & df2["Unique Opens"].apply(validate_dtype).all()
                & df2["Clicks"].apply(validate_dtype).all()
                & df2["Unsubs"].apply(validate_dtype).all()
                & df2["sb"].apply(validate_dtype).all()
                & df2["hb"].apply(validate_dtype).all()
            ):
    ...
    if df["timestamp_append"][0] == "Y":
        if df["timestamp_report_path"].str.contains("/").all():
            if os.path.isfile(df["timestamp_report_path"][0]):
                ...
                if (
                    (
                        pd.to_datetime(df4.iloc[:, 0])
                        == pd.to_datetime(df4.iloc[:, 1]).dt.date
                    )
                    & (
                        pd.to_datetime(df4.iloc[:, 1]).dt.date
                        == pd.to_datetime(df4.iloc[:, 2]).dt.date
                    )
                ).all():
    ...
    if os.path.isfile(df["decile_wise_report_path"][0]):
        df3 = pd.read_csv(
            df["decile_wise_report_path"][0], sep="|", header=None, thousands=","
        )
        if len(df3.columns) == 8:
            ...
            if os.path.isfile(df["cpm_report_path"][0]):
                ...
                try:
                    ...
                    # =======OLD================#
                    if (
                        df3["old_delivered_per"]
                        .drop_duplicates()
                        .sort_values()
                        .reset_index(drop=True)
                        .all()
                        > 0
                        and df3["old_delivered_per"]
                        .drop_duplicates()
                        .sort_values()
                        .reset_index(drop=True)
                        .notna()
                ...
                if (
                    pd.to_datetime(df["residual_date"])
                    >= pd.to_datetime(df2["Date"].max())
                ).any():
            ...
            if (
                df3["Delivered"].apply(validate_dtype).all()
                & df3["Opens"].apply(validate_dtype).all()
                & df3["clicks"].apply(validate_dtype).all()
                & df3["unsubs"].apply(validate_dtype).all()
            ):
            ...
            cpm_report_result = (
                df2.groupby(["segment", "sub_seg"])["Delivered"]
                .sum()
                .rename_axis(index=["segment", "subseg"])
                .reset_index()
                .sort_values(by=["segment", "subseg"])
            )
            # print("Test2")
            del_report_result = (
                df3.groupby(["segment", "sub_seg"])["Delivered"]
                .sum()
                .rename_axis(index=["segment", "subseg"])
                .reset_index()
                .sort_values(by=["segment", "subseg"])
            ...
            comparison = pd.merge(
                ...
                on=["segment", "subseg"],
                how="outer",
                suffixes=("_cpm", "_decile"),
            )
            comparison["difference"] = (
                comparison["Delivered_cpm"] - comparison["Delivered_decile"]
            )
    ...
    if df["type"][0] == 2:
        if os.path.isfile(df["unique_decile_report_path"][0]):
            df4 = pd.read_csv(
                df["unique_decile_report_path"][0], sep="|", header=None, thousands=","
            )
            if len(df4.columns) == 8:
                ...
                try:
                    if (
                        ...
                        validation_status.append("Failed")
                    if (
                        df4["old_delivered_per"]
                        .drop_duplicates()
                        .sort_values()
                        .reset_index(drop=True)
                        .all()
                        > 0
                        and df4["old_delivered_per"]
                        .drop_duplicates()
                        .sort_values()
                        .reset_index(drop=True)
                        .notna()
                ...
                if (
                    df4["Delivered"].apply(validate_dtype).all()
                    & df4["Opens"].apply(validate_dtype).all()
                    & df4["clicks"].apply(validate_dtype).all()
                    & df4["unsubs"].apply(validate_dtype).all()
                ):
                    # print('All valid decile')
    ...
    try:
        ...
        for query in qry:
            if re.findall("apt_rltp_request_raw_", query):
                current_ids.append(
                    re.findall("apt_rltp_request_raw_(\d+)_postback_file", query)[0]
                )
                if (
                    int(
                        re.findall("apt_rltp_request_raw_(\d+)_postback_file", query)[0]
        ...
        PreviousID = pd.read_sql(
            "select request_id,query,request_status from  APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND where client_id="
            + str(df["client_id"][0])
            + "  and request_status='C' order by request_id  desc LIMIT 1 OFFSET 1",
            con=engine,
        )
        ...
        if not PreviousID.empty and int(df6["weeklynewrltpid"][0]) == 1:
            ...
            for prev_qry in prev_query:
                if re.findall("apt_rltp_request_raw_", prev_qry):
                    if (
                        re.findall(
                            "apt_rltp_request_raw_(\d+)_postback_file", prev_qry
                        )[0]
                        in current_ids
                    ):
...
if "Failed" in validation_status or Qs == True:
    ...
    print(dfd)
    cur.execute(
        "update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_validation='N' where request_id="
        + sys.argv[1]
        + ""
    )
    styles = [
        {
            "selector": "th, td",
            "props": [("white-space", "nowrap"), ("border", "1px solid black")],
        },
        {
            "selector": "th",
            "props": [
    ...
    html_table = (
        dfd.style.applymap(style_cell, subset=["Validation Status"])
        .applymap(style_first_row, subset=["Validation Case"])
    ...
    cur.execute(
        "update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_validation='Y',from_date="
        + "'"
        + str(df2["Date"].min())
        + "'"
        ",end_date=" + "'" + str(df2["Date"].max()) + "'"
        " where request_id=" + sys.argv[1] + ""
    )
