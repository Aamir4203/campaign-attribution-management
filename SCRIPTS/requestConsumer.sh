...
source /u1/techteam/PFM_CUSTOM_SCRIPTS/Campaign-Attribution-Management/SCRIPTS/config.properties
...
$CONNECTION_STRING  --no-align --field-separator '|'  --pset footer -qtAX -c "with cte as (select actuallogscount,actuallogstrtmatchcount,actuallogspbreportedcount,actualopenscount,openstrtmatchcount,openspbreportedcount,actualclickscount,clickstrtmatchcount,clickspbreportedcount,openstoopenspbreportedgencount,clickstoclickspbreportedgencount from $REQUEST_TABLE a join $CLIENT_TABLE b on a.CLIENT_ID=b.CLIENT_ID  join $QA_TABLE c on a.REQUEST_ID=c.REQUEST_ID where  a.REQUEST_ID=$new_request_id) select x.* from cte cross join lateral (values
	...
	sh $SCRIPTPATH/sendMail2.sh "$new_request_id"

}

# Function to setup request working directories and copy scripts
setup_request_environment() {
    local request_id=$1

    # Create directories with -p flag (safe if exists)
    mkdir -p "$HOMEPATH"
    mkdir -p "$HOMEPATH/FILES"
    mkdir -p "$HOMEPATH/SPOOL"
    mkdir -p "$HOMEPATH/LOGS"
    mkdir -p "$HOMEPATH/ETC"
    mkdir -p "$SCRIPTPATH"

    # Copy configuration and scripts with error handling
    if ! cp "$MAIN_SCRIPTS/config.properties" "$HOMEPATH/ETC/"; then
        echo "ERROR: Failed to copy config.properties for request_id=$request_id"
        $CONNECTION_STRING -vv -c "update $REQUEST_TABLE set request_status='E',request_desc='Setup failed: config copy error' where request_id=$request_id"
        exit 1
    fi

    cp "$MAIN_SCRIPTS"/*.sh "$SCRIPTPATH/" 2>/dev/null
    cp "$MAIN_SCRIPTS"/*.py "$SCRIPTPATH/" 2>/dev/null
    cp "$MAIN_SCRIPTS"/*.jar "$SCRIPTPATH/" 2>/dev/null

    # Create request-specific copies
    cp "$MAIN_SCRIPTS/consumerDeliveredScript.sh" "$SCRIPTPATH/consumerDeliveredScript_${request_id}.sh"
    cp "$MAIN_SCRIPTS/respondersPulling.sh" "$SCRIPTPATH/respondersPulling_${request_id}.sh"
}
...
subseg=`$CONNECTION_STRING -qtAX -c "select UPPER(SUB_SEG) from  $REQUEST_TABLE where REQUEST_ID=$new_request_id"`
...
then
    ...
    then
        ...
        if [[ $? -ne 0 ]]
        then
            echo -e " Hi Team, \n Unable to delete from client unsub table for re-work. \n\n Thanks, \n SysAdmin" | mail -s "APT REQUEST DETAILS :: $CLIENT_NAME " "$ALERT_TO"
...
then
    ...
    then
        ...
        sh -x "$SCRIPTPATH/respondersPulling.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
        sh -x "$SCRIPTPATH/suppressionList.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
    fi
    ...
    then
        ...
        sh -x "$SCRIPTPATH/srcPreparation.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
    fi
    ...
    then
        ...
        sh -x "$SCRIPTPATH/partitioningSrc.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1

        get_status=`$CONNECTION_STRING -qtAX -c "select upper(REQUEST_STATUS) from $REQUEST_TABLE where REQUEST_STATUS='E' and REQUEST_ID=$new_request_id "`
        ...
        sh -x "$SCRIPTPATH/deliveredScript.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
    ...
    then
        ...
        sh -x "$SCRIPTPATH/timestampAppending.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
        ...
        ip_append=`$CONNECTION_STRING -qtAX -c "select upper(IP_APPEND) from  $REQUEST_TABLE where REQUEST_ID=$new_request_id"`

        if [[ $ip_append == 'Y' ]]
        then
            sh -x "$SCRIPTPATH/ipAppending.sh" "$new_request_id" >>"$HOMEPATH/LOGS/${new_request_id}.log" 2>&1
            get_status=`$CONNECTION_STRING -qtAX -c "select upper(REQUEST_STATUS) from $REQUEST_TABLE where REQUEST_STATUS='E' and REQUEST_ID=$new_request_id "`
