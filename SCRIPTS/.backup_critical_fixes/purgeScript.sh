...
#-------------------BACKUP_APT_TABLES---------------------#
		gzip $BKP_PATH/$GEN_TABLE $BKP_PATH/$POSTBACK_TABLE 
                ...
                /usr/local/bin/aws s3 cp $BKP_PATH/$GEN_TABLE.gz      s3://new-datateam/Campaign-Attribution-Management/$REQUEST_ID/

                /usr/local/bin/aws s3 cp $BKP_PATH/$POSTBACK_TABLE.gz s3://new-datateam/Campaign-Attribution-Management/$REQUEST_ID/
...
#------------------ UPDATING PURGING DETAILS -------------#
		...
		then
	       ...
	       # rm -rf /u1/techteam/PFM_CUSTOM_SCRIPTS/Campaign-Attribution-Management/REQUEST_PROCESSING/$REQUEST_ID
		       
	       if [ -d "/u1/techteam/PFM_CUSTOM_SCRIPTS/Campaign-Attribution-Management/REQUEST_PROCESSING/$REQUEST_ID" ]
	       then
		       rm -rf "/u1/techteam/PFM_CUSTOM_SCRIPTS/Campaign-Attribution-Management/REQUEST_PROCESSING/$REQUEST_ID"
